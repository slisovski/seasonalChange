...
medDat <- do.call("rbind", mclapply(batchID[batchID%in%inBBB], function(x) {
...
datR <- do.call("stack", mclapply(2:3, function(x) {
...
plot(landC, col = "grey90", border = NA)
plot(pol[batchID[batchID%in%inBBB],], col = adjustcolor("orange", 0.1), add = T)
