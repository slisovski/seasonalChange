...
batch <- batchID[as.numeric(args[1])]
    ...
    distM <- suppressMessages(geodist(st_coordinates(st_crds), measure = "cheap"))

    pxlPhen <- lapply(which(onLand), function(pxl) {
    ...
    drive_upload(paste0("/work/ollie/slisovsk/Projects/SeasonalChange/Results/phenBatches/phenBatch_", batch,".rda"),
                 path = "SeasonalChange/phenBatches/", name = paste0("phenBatch_", batch, ".rda"), verbose = FALSE, overwrite = T)
